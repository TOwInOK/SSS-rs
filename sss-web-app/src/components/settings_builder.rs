...
pub fn SettingsBuilder() -> impl IntoView {
    ...
    view! {
        <div class="grid grid-cols-1 gap-4 h-full md:max-h-dvh md:overflow-y-scroll md:will-change-scroll">
            <Show when=move || limitations.get().is_user_section_allowed()>
                <UserSection />
            </Show>
            <Show when=move || limitations.get().is_about_section_allowed()>
                <AboutSection />
            </Show>
            <Show when=move || limitations.get().is_specification_section_allowed()>
                <SpecificationsSection />
            </Show>
            <Show when=move || limitations.get().is_repositories_section_allowed()>
                <RepositoriesSection />
            </Show>
            <Show when=move || limitations.get().is_socials_section_allowed()>
                <SocialsSection />
            </Show>
            <Show when=move || limitations.get().is_skills_section_allowed()>
                <SkillsSection />
            </Show>
        </div>
    }
...
pub fn UserSection() -> impl IntoView {
    ...
    view! {
        <SectionInverted title="User section">
                ...
                <ScrollableBox>
                    <Input
                        ...
                        action=move |ev| {
                            set_settings
                                .update(|x| {
                                    x.layout.user.current_nickname.word = ev.target().value();
                                });
                        }
                    ...
                    <Input
                        ...
                        action=move |ev| {
                            set_settings
                                .update(|x| {
                                    x.layout.user.current_nickname.pronounce = ev.target().value();
                                });
                        }
            ...
            <Show when=move || limitations.read().is_user_prevision_nicknames_section_allowed()>
                    ...
                    <ScrollXBar>
                            ...
                            <ScrollableBox>
                                <Input
                                    ...
                                    action=move |ev| {
                                        set_settings
                                            .update(|x| {
                                                x.layout.user.prevision_nicknames[index].word = ev
                                                    .target()
                                                    .value();
                                            });
                                    }
                                ...
                                <Input
                                    ...
                                    action=move |ev| {
                                        set_settings
                                            .update(|x| {
                                                x.layout.user.prevision_nicknames[index].pronounce = ev
                                                    .target()
                                                    .value();
                                            });
                                    }
                                ...
                                <Button
                                    ...
                                    action=move || {
                                        set_settings
                                            .update(|x| {
                                                x.layout.user.prevision_nicknames.remove(index);
                                            });
                                    }
                        ...
                        <Show when=move || {
                            settings.read().layout.specifications.len()
                                < limitations.read().user_prevision_nicknames_max_count()
                        }>
                            <Button
                                ...
                                action=move || {
                                    set_settings
                                        .update(|x| {
                                            x.layout.user.prevision_nicknames.push(Nickname::default())
                                        });
                                }
